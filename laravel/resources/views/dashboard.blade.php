...
<body class="h-screen overflow-hidden">
    ...
    <div id="toast-container" class="fixed bottom-4 right-4 z-[70] space-y-2 pointer-events-none">
        @if(session('success'))
    ...
    <div class="flex justify-center p-2 h-[calc(100vh-40px)] overflow-hidden mb-4">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="flex flex-wrap items-center gap-1">
                            ...
                            <form method="GET" action="{{ route('dashboard') }}" class="flex flex-wrap gap-1 flex-1 min-w-[200px]">
                                <input type="text" name="search" placeholder="Search..." value="{{ request('search') }}" class="flex-1 border-2 border-gray-300 rounded-l-lg px-2 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-xs transition-all duration-200 shadow-sm">
                    ...
                    <tbody>
                        ...
                        <tr class="{{ $index % 2 == 0 ? 'bg-gray-50' : 'bg-white' }} hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 transition-colors duration-200 border-b border-gray-100">
                            ...
                            <td class="px-3 py-2 text-[10px] whitespace-nowrap">
                                @php
                                // 1. Check database condition first (which we auto-updated in Controller)
                                $condition = $item->condition;

                                // 2. Visual Check: Is it expired?
                                $isExpired = false;
                                if($item->expiry_date) {
                                $isExpired = \Carbon\Carbon::parse($item->expiry_date)->isPast();
                                }

                                // 3. Force UI to show Unserviceable if expired (Double check)
                                if ($isExpired) {
                                $condition = 'unserviceable';
                                }

                                $isUnserviceable = ($condition === 'unserviceable');
                                @endphp
        ...
        @endif">
                                    ...
                                    @if($isExpired)
                                    <i class="fas fa-hourglass-end ml-1" title="Expired"></i>
                                    @endif
                                </span>
                            </td>
                            ...
                            <td class="px-3 py-2 text-[10px] whitespace-nowrap">
                                @if($item->expiry_date)
                                @php
                                // Parse the explicit date from the database
                                $expirationDate = \Carbon\Carbon::parse($item->expiry_date);

                                // Check if today is past that date
                                $isExpired = now()->startOfDay()->greaterThan($expirationDate->startOfDay());
                                @endphp
        ...
        <div id="itemDetailsModal" class="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm hidden items-center justify-center z-[60] transition-opacity duration-300">
        ...
        <div id="deleteModal" class="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm hidden items-center justify-center z-[60] transition-opacity duration-300">
        ...
        <script>
            ...
            // --- Updated Compact Item Details Modal ---
        ...
        <div id="addItemModal" class="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm hidden flex items-center justify-center z-[60] transition-opacity duration-300 p-4">
                ...
                <div class="p-6">
                    <form action="{{ route('items.store') }}" method="POST" class="space-y-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <label for="expiry_date" class="block text-sm font-medium text-gray-700 mb-2">Date Expiry (dd/mm/yyyy)</label>
        ...
        <div id="transferModal" class="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm hidden flex items-center justify-center z-[60] transition-opacity duration-300 p-4">
        ...
        <div id="editItemModal" class="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm hidden flex items-center justify-center z-[60] transition-opacity duration-300 p-4">
...
// Logic to auto-update Condition based on Expiry Date for ADD MODAL
    const addExpiryInput = document.getElementById('expiry_date');
    const addConditionSelect = document.getElementById('condition');

    if (addExpiryInput && addConditionSelect) {
        addExpiryInput.addEventListener('change', function() {
            ...
            // Logic: Past = Unserviceable, Future = Serviceable
            if (expiryDate < today) {
                addConditionSelect.value = 'unserviceable';
            } else {
                addConditionSelect.value = 'serviceable';
            }
        });
    }
    ...
    // 1. Existing Total Cost Logic
    const quantityInput = document.getElementById('edit_quantity_on_hand'); // Ensure ID matches your edit form
    const unitCostInput = document.getElementById('edit_unit_cost');        // Ensure ID matches your edit form
    
    // Note: If your edit form IDs are just 'quantity_on_hand' and 'unit_cost', use those instead
    const genericQty = document.querySelector('#editFormDisplay input[name="quantity_on_hand"]');
    const genericCost = document.querySelector('#editFormDisplay input[name="unit_cost"]');

    if (genericQty && genericCost) {
        genericQty.addEventListener('input', calculateEditTotalCost);
        genericCost.addEventListener('input', calculateEditTotalCost);
    }

    // 2. NEW FIX: Auto-update Condition based on Expiry Date
    const expiryInput = document.querySelector('#editFormDisplay input[name="expiry_date"]');
    const conditionSelect = document.querySelector('#editFormDisplay select[name="condition"]');

    if (expiryInput && conditionSelect) {
        expiryInput.addEventListener('change', function() {
            ...
            // If date is in the past, force Unserviceable. 
            // If future, force Serviceable.
            if (expiryDate < today) {
                conditionSelect.value = 'unserviceable';
            } else {
                conditionSelect.value = 'serviceable';
            }
        });
    }
