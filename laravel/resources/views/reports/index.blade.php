    ...
    <body class="h-screen overflow-hidden">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-1">
                                <div class="bg-gradient-to-r from-blue-50 to-indigo-50 p-2 rounded-lg border-l-2 border-blue-500">
                                    ...
                                    <select name="type" id="type" class="w-full border border-gray-300 rounded px-2 py-1 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 shadow-sm bg-white text-xs">
                                        ...
                                        <option value="addition" {{ request('type') == 'addition' ? 'selected' : '' }}>Added</option>
                                        <option value="decrease" {{ request('type') == 'decrease' ? 'selected' : '' }}>Removed</option>
                    ...
                    <table class="w-full table-fixed">
                        <thead class="sticky top-0 z-10">
                            <tr class="bg-gradient-to-r from-blue-600 via-blue-700 to-blue-600 text-white shadow-lg">
                                <th class="px-3 py-1.5 text-left text-[10px] font-bold uppercase tracking-wider w-36">
                                    <i class="fas fa-box mr-1.5 text-[10px]"></i>Item
                                </th>
                                <th class="px-3 py-1.5 text-left text-[10px] font-bold uppercase tracking-wider w-24">
                                ...
                                <th class="px-1 py-1.5 text-center text-[10px] font-bold uppercase tracking-wider w-16">
                                    <i class="fas fa-plus mr-1 text-[10px]"></i>New
                                </th>
                                <th class="px-3 py-1.5 text-left text-[10px] font-bold uppercase tracking-wider w-28">
                        ...
                        <tbody>
                            ...
                            <tr class="{{ $index % 2 == 0 ? 'bg-gray-50' : 'bg-white' }} hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 transition-colors duration-200 border-b border-gray-100">
                                <td class="px-3 py-2 text-[9px] text-gray-900 font-semibold w-36 truncate">
                                    <i class="fas fa-box-open mr-2 text-blue-500"></i>{{ optional($report->item)->name ?? $report->item_name }}
                                </td>
                                <td class="px-3 py-2 text-[9px] text-gray-700 whitespace-nowrap w-24">
                                    <i class="fas fa-clock mr-2 text-gray-400"></i>{{ $report->created_at->format('M d, H:i') }}
                                </td>
                                <td class="px-3 py-2 text-[9px] w-24">
                                    <span class="w-24 inline-block px-3 py-1.5 rounded-full text-[9px] font-bold shadow-sm
                                        @if($report->type == 'addition') bg-gradient-to-r from-green-400 to-emerald-500 text-white
                                        @elseif($report->type == 'decrease') bg-gradient-to-r from-red-400 to-rose-500 text-white
                                        @elseif($report->type == 'transfer') bg-gradient-to-r from-blue-600 via-blue-700 to-blue-600 text-white
                                ...
                                <td class="px-1 py-2 text-[9px] text-gray-700 font-medium text-center w-16">
                                    <div class="flex items-center justify-center">
                                        <div class="bg-gray-100 border border-gray-200 text-gray-600 px-1.5 py-0.5 rounded shadow-sm min-w-[30px]">
                                            {{ $report->prev_quantity ?? 'N/A' }}
                                        </div>
                                    </div>
                                </td>
                                <td class="px-1 py-2 text-[9px] text-gray-900 font-bold text-center w-16">
                                    @if($report->type == 'addition')
                                ...
                                <td class="px-1 py-2 text-[9px] text-gray-700 font-medium text-center w-16">
                                    <div class="flex items-center justify-center">
                                        <div class="bg-gray-100 border border-gray-200 text-gray-600 px-1.5 py-0.5 rounded shadow-sm min-w-[30px]">
                                            @if($report->prev_quantity !== null)
                                                @if($report->type == 'addition')
                                                    {{ $report->prev_quantity + $report->quantity_change }}
                                                @elseif($report->type == 'decrease')
                                                    {{ $report->prev_quantity - $report->quantity_change }}
                                                @else
                                                    {{ $report->prev_quantity }}
                                                @endif
                                            @else
                                                N/A
                                            @endif
                                ...
                                <td class="px-3 py-2 text-[9px] font-bold whitespace-nowrap w-28">
                                    @if($report->cost_change > 0)
                                    <span class="text-green-700 bg-green-50 px-2 py-1 rounded-full text-[9px] truncate">
                                        <i class="fas fa-arrow-up mr-1"></i>+₱{{ number_format($report->cost_change, 2) }}
                                    </span>
                                    @else
                                    <span class="text-red-700 bg-red-50 px-2 py-1 rounded-full text-[9px] truncate">
                                        <i class="fas fa-arrow-down mr-1"></i>-₱{{ number_format(abs($report->cost_change), 2) }}
                                    </span>
                                    @endif
                                </td>
                                <td class="px-3 py-2 text-[9px] text-gray-700 w-36 truncate">
                                    <i class="fas fa-comment-alt mr-2 text-gray-400"></i>{{ $report->reason ?? 'N/A' }}
                                </td>
                                <td class="px-3 py-2 text-[9px] text-gray-700 whitespace-nowrap w-28">
                                    <i class="fas fa-user-circle mr-2 text-gray-400"></i>{{ $report->user->name }}
                                </td>
                                <td class="px-3 py-2 text-xs whitespace-nowrap w-24">
                            ...
                            @empty
                            <tr>
                                <td colspan="10" class="px-6 py-16 text-center bg-gradient-to-br from-gray-50 to-gray-100">
        ...
        <script>
            ...
            document.getElementById('sidebarOverlay').addEventListener('click', function() {
                document.getElementById('sidebar').classList.add('translat  e-x-full');
                document.getElementById('sidebarOverlay').classList.add('hidden');
            });
